...
LOG.info("searching ");
				...
				org = SecureTeacherSchoolManager.getStudentsInSchool(rest, new DomSchoolOrganisation());
				List<DomUser> students = org.getUsers();
LOG.info("found students " + students.size());
				Optional<DomUser> b1 = students.stream().filter(s -> s.getId().toString().equals(split[1])).findAny();
LOG.info("student " + b1);
				List<DomSchoolClass> classes = org.getSchoolClasses();
...
LOG.info("class " + b2);
				if (b1.isPresent() && b2.isPresent()) {
					...
					Stream<DomHasRole> x = (org.getHasRoles().stream().filter(hr -> hr.getUserId().equals(b1.get().getId())));
					result.setHasRole(x.findAny().get());
