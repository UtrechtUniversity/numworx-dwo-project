...
public class FormuleEditorWithSteps  implements InteractionView
{
	...
	{	this.randomVarNamen = randomVarNamen;
		...
		{	int index = antwoordString.indexOf("=");
	        if(index==-1) index = antwoordString.indexOf("\u2248");
			if(index>-1)
	...
	{	
		...
		if(hasPrefix)current.remove(prefixViewer);
		...
		current.add(p);
		if(hasFeedback)contentPanel.add(feedbackPanel);
		nagekeken = true;
		correct = true;
		score = scoreMax;
		comRoot.setChanged();
	...
	{	contentPanel.remove(feedbackPanel);
		...
		if(!isVergelijkingVak && !hasPrefix && (useranswer.charAt(useranswer.length()-2))!='=')useranswer = useranswer.substring(0,useranswer.length()-1) + "=@";
		//System.out.println(" useranswer2: "+ useranswer);
		FormuleViewer fv = new FormuleViewer(prefix.substring(2,prefix.length()-1)+ useranswer.substring(2,useranswer.length()-1));
		//System.out.println(" useranswer3: "+ useranswer);
		
		fv.showResult(fv.ALMOSTCORRECT);
		if(latest_answer_viewer!=null && !(hasStartString && steps==1))latest_answer_viewer.showResult(fv.NONE);
		latest_answer_viewer = fv;
		viewers.add(fv);
		Panel p = fv.getAsPanel();
		p.getElement().getStyle().setProperty("display", "inline");
		...
		if(hasPrefix)stepPanel.add(prefixViewer);
	...
	{	
		if(!correct)
		{
			...
			{	editor.getMainRegel().deleteAll();
				...
				if(hasPrefix)currentTekst = removePrefix(currentTekst);
	...
	{	FlowPanel current = stepPanels.get(steps);
		...
		{	contentPanel.remove(feedbackPanel);
			...
			editor = addNewEditor(current);
			editor.insert(currentTekst);
			if(steps>1 || steps>0 && !hasStartString)latest_answer_viewer = viewers.get(viewers.size() - 1);
			correct = false;
			comRoot.setChanged();
		}
		...
		{	contentPanel.remove(feedbackPanel);
			...
			current.remove(viewers.get(viewers.size() - 1).getAsPanel());
			viewers.remove(viewers.get(viewers.size() - 1));
			if(steps>2 || steps>1 && !hasStartString)latest_answer_viewer = viewers.get(viewers.size() - 1);
	...
	{	mainPanel = new FlowPanel();
		...
		{	if(!isVergelijkingVak && !hasPrefix && (startString.charAt(startString.length()-2))!='=')startString = startString.substring(0,startString.length()-1) + "=@";
			...
			if(hasPrefix)stepPanelNew.add(prefixViewer);
	...
	{	FormuleEditorWithAnswer editor = new FormuleEditorWithAnswer(h, isVergelijkingVak, this, randomVarNamen, randomVarWaarden);
		if(!hasPrefix)editor.getAsPanel().getElement().getStyle().setMarginLeft(13, Unit.PX);
		...
		kb.setEditor(editor);
		if(hasPrefix)p.add(prefixViewer);
		p.add(tp);
	...
	{	
		...
		{	if(viewers.size()>i && viewers.get(i) != null)formuleVakInhouden[i] = (viewers.get(i)).toString();
			else formuleVakInhouden[i] = "";
		}
	...
	{	
		...
		if(h.get("stapNr") != null) stapNr = (Integer)h.get("stapNr");
		if(h.get("ingevuld") != null) ingevuld = (Boolean)h.get("ingevuld");
		if(h.get("nagekeken") != null) nagekeken = (Boolean)h.get("nagekeken");
		if(h.get("formuleVakInhouden") != null) formuleVakInhouden = (String[])h.get("formuleVakInhouden");
		if(h.get("antwoordString") != null) antwoordString = (String)h.get("antwoordString");
		...
		for (int i = 0; i < steps+1; i++)
		{	if(i==0 && hasStartString) i++;
			
			FormuleViewer fv = new FormuleViewer(formuleVakInhouden[i]);
			if(i==steps && nagekeken)fv.showResult(FormuleViewer.CORRECT);
			else if(i==steps-1 && !nagekeken)fv.showResult(FormuleViewer.ALMOSTCORRECT);
			else fv.showResult(FormuleViewer.NONE);
			viewers.add(fv);
			Panel p = fv.getAsPanel();
			p.getElement().getStyle().setProperty("display", "inline");
			...
			{	stepPanel = stepPanels.get(i);
				...
				if(hasPrefix && (i<steps || nagekeken)) stepPanel.remove(prefixViewer);
			}
			else 
			{	stepPanel = new FlowPanel();
				layoutStepPanel(stepPanel);
				stepPanels.add(stepPanel);
				if(hasPrefix && !nagekeken)stepPanel.add(prefixViewer);
			...
			if(viewers.size()>0)latest_answer_viewer = viewers.get(viewers.size()-1);
			correct = nagekeken;
		...
		if(steps>0 || steps==0 && !hasStartString)tb.getElement().getStyle().setVisibility(Visibility.VISIBLE);
