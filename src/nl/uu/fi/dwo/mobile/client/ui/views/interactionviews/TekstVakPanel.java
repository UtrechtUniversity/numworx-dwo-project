...
import com.bouwkamp.gwt.user.client.ui.*;
...
public class TekstVakPanel implements InteractionView
{
	...
	public TekstVakPanel(HashMap<String, Object> h, String[] randomVarNamen, HashMap randomVarWaarden)
	{
		...
		if(breedtes.size()>1)mainPanel.getElement().getStyle().setProperty("margin",""+(-cellSpaceRow)+"px "+(-cellSpaceColumn)+"px");
		...
		{	for(int j=0 ; j<breedtes.size() ; j++)
			{	tekstVakken[i][j] = new FlowPanel();
				if(bgColorZichtbaar)tekstVakken[i][j].getElement().getStyle().setBackgroundColor(bgColor.toString());
