package nl.uu.fi.dwo.mobile.client.ui.views.interactionviews.nabouwenaanzichtengwt.client;
...
public class NabouwenAanzichtenGWT implements InteractionView {
	Canvas canvas;
	int mouseX, mouseY;
	OpdrNav comRoot;
	...
	private void check(){
		if(vWerk==null || !ingevuld ||!kijkNaActief)return;
	...
	void zetVeranderd() {
		if(vWerk==null || !kijkNaActief)return;
		nakijkKnop.clear();
		nakijkKnop.add(new Image("images/resources/vinkjegrijs.png"));
		correct = false;
		score = 0;
		if(!startKr.isGelijk(vWerk.kr))ingevuld = true;
		else ingevuld = false;
		comRoot.setChanged();
	}
