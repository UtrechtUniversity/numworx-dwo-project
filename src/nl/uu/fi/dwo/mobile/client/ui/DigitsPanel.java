...
import lx.interaction.touch.GWTMouseHandler;
import lx.interaction.touch.GWTTouchHandler;
...
public class DigitsPanel extends HorizontalPanel implements TouchListener, DollarListener {
	...
	private void initialize(int width, int height) {
		...
		if( !hastouch ) {
		GWTMouseHandler mouser = new GWTMouseHandler(this);
			canvas.addMouseDownHandler(mouser);
			canvas.addMouseUpHandler(mouser);
			canvas.addMouseMoveHandler(mouser);
		} else {
		GWTTouchHandler toucher = new GWTTouchHandler(this);
			canvas.addTouchStartHandler(toucher);
			canvas.addTouchMoveHandler(toucher);
			canvas.addTouchEndHandler(toucher);
		}
	}
	...
	public void dollarDetected(Dollar dollar) {
		GWT.log("detected " + dollar.getName() + " " + dollar.getScore());	
