...
public class TreeModuleViewImplTablet  extends Composite implements TreeModuleView, ViewModuleView.Loader
{
	...
	public void onFullscreenTap(TapEvent event) {
	...
	*/
	...
	public void toggleFullScreen() {
		...
		if (modulePanel.getAbsoluteTop() == 0){
			// go to full screen
			slideNavigationToTopAnimation anim= new slideNavigationToTopAnimation(-40);
			this.fullscreenButton.getElement().setInnerHTML("<span class='fa fa-compress'></span>");
			anim.run(animation_duration);
			
			// also slide out the navigation area 
			slideNavigationOut();
			
			
		} else if (modulePanel.getAbsoluteTop() == -40) {
			
			slideNavigationToTopAnimation anim= new slideNavigationToTopAnimation(40);
			this.fullscreenButton.getElement().setInnerHTML("<span class='fa fa-expand'></span>");
			anim.run(animation_duration);
			
			// if SlideIn the left area if needed 
			slideNavigationIn();		
		}
	...
	*/ 
	...
	public class slideNavigationToTopAnimation  extends Animation {
        // initial offset of the Panels
        private int startOffsetNavigation = 0;
        private int startOffsetModule = 0;
        ...
        public slideNavigationToTopAnimation(int desiredOffset) {
    	
        	this.startOffsetNavigation =  navigationPanel.getAbsoluteTop();
        	this.startOffsetModule = modulePanel.getAbsoluteTop();
        ...
        protected void onUpdate(double progress) {
        	...
        	//  move the navigation panel
            double offset =  (this.startOffsetNavigation  + (this.desiredOffset * progress)) ;
            navigationPanel.getElement().getStyle().setTop(offset, Unit.PX);
            
            // move the module Panel
            offset =  (this.startOffsetModule  + (this.desiredOffset * progress))  ;
            modulePanel.getElement().getStyle().setTop(offset, Unit.PX);
