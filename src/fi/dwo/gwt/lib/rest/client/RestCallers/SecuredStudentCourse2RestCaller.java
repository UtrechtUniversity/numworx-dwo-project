...
public class SecuredStudentCourse2RestCaller implements CoursesOfSchoolRestCaller {
	...
	public interface Helper extends RestService  {
	    @PUT
	...
	static Helper REST = GWT.create(Helper.class);
	
	@Override
	public void getCoursesClass(RestSchoolClassAndProfile rest, MethodCallback<DomCoursesOfSchoolClass> callback) {
		String id = PathId.getId(rest.getRestContext());
		REST.getCoursesClass(id, rest, callback);
	}

	@Override
	public void getCoursesClass(RestCourse rest, MethodCallback<DomCoursesOfSchoolClass> callback) {
		String id = rest.getRestContext().getDomHasRole().getId().getIdString();
		REST.getCoursesClass(id, rest, callback);
	}

	@Override
	public void getCoursesClass(RestScoContext rest, MethodCallback<DomCoursesOfSchoolClass> callback) {
		String id = rest.getRestContext().getDomHasRole().getId().getIdString();
		REST.getCoursesClass(id, rest, callback);
	}
