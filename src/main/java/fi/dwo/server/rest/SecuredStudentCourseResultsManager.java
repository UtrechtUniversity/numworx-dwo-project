...
import nl.uu.fi.dwo.rest.entities.RestStudentCourseResultsRequest;
 ...
 */
@PermitAll
@Path("/secure/student/results")
public class SecuredStudentCourseResultsManager extends AbstractSchoolClassManager {

    private static final Logger LOG = Logger.getLogger(SecuredStudentCourseResultsManager.class.getName());
    ...
    @Path("/getStudentCourseResults")
    public DomResultsPerStudentCourse getStudentCourseResults(@Context SecurityContext sc, RestStudentCourseResultsRequest req) {
    	DomResultsPerStudentCourse results = null;
    	
// check for stuydent's hasrole mentioned in RestContext
// check if course profile is allowed and matches given profile
// return data    	
    	
        DomCourse domCourse = req.getDomCourse();
        DomContext context = req.getRestContext();
        ...
        try {
            ...
            course = CourseManager.findEntity(MySQLPersistenceId.getNativeId(domCourse));
            if (course == null) {
                LOG.log(Level.SEVERE, "Username {0}: ILLEGAL USER-OPERATION: Using unknown profileId {1} in HasRole differs from user principal name {0}.", new Object[]{sc.getUserPrincipal().getName(), domCourse.getId()});
                throw new Dwo2Exception(Dwo2ExceptionCode.User_IllegalAction, "You Don't Have Permission to access this using usercode " + sc.getUserPrincipal().getName() + ".");
            }
