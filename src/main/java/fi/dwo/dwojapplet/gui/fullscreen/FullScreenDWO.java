...
public class FullScreenDWO extends JDialog implements ChangeListener,
		ActionListener, InternalFrameListener {

	private final String CONFIRM_TITLE = TextMapper
			.getText(TextMapper.FSD_CONFIRM_TITLE);
	...
	public void initialize(JComponent component) {
		...
		setTitle(TextMapper.getText(TextMapper.FSD_TITLE));
		// setResizable(false);
		setUndecorated(true); // //?????
		...
		JInternalFrame frame = new JInternalFrame("Logging", true, false, true,
				true);
		...
		String name = u.getUsername() + " " + u.getName();
		JLabel nametag = new JLabel(name);
		nametag.setFont(new Font("Serif", 0, 48));
		nametag.setSize(nametag.getPreferredSize());
		nametag.setLocation(20, 10);
		content.add(nametag);
		frame = new JInternalFrame(
				TextMapper.getText(TextMapper.FSD_ACTIVITEIT), true, true,
				true, true);

		// hier is de juiste maat van de activiteit nodig (zonder
		// scrollbalken!!)
		// nu even hard
		component.setBounds(20, 110, 1024, 500);
		frame.setBounds(component.getBounds()); // initiele maten.....
		Insets insets = frame.getInsets();
		frame.setSize(frame.getWidth() + 10 + insets.left + insets.right + 30,
				insets.top + insets.bottom + frame.getHeight() + 140); // marge

		// Snelle fix: alles absoluut gepositioneerd. later beter maken
		BackgroundPanel p = new BackgroundPanel(new BorderLayout());

		// p.setOpaque(false);
		p.setGuiImage(DwoHelper.getImage(GuiConstants.RESOURCES
				+ GuiConstants.GUI_IMAGE_SCO));
		p.add(component, BorderLayout.CENTER);
		p.setBorder(BorderFactory.createEmptyBorder(0, 5, 10, 5)); // met die 0
																	// kan ik de
																	// knop
																	// laten
																	// zakken
		...
		pp.setOpaque(false);
		pp.add(button = new JButton(TextMapper
				.getText(TextMapper.FSD_AFSLUITEN)));
		...
		Dimension screenSize = DwoHelper.getApplet().getToolkit()
				.getScreenSize();
		...
		if (source instanceof WindowEvent) {
			...
			if (we.getID() == WindowEvent.WINDOW_DEACTIVATED && !fuse) {
				...
				if (w != null) {
					...
					Frame mainFrame = JOptionPane
							.getFrameForComponent(DwoHelper.getApplet());
					if (w == mainFrame || c == this || c == mainFrame
						...
						// monitor w for deactivate events met NULL als
						// opposite.
				...
				vbox.add(new JLabel(TextMapper.getText(TextMapper.FSD_SURE)));
				...
				int r = JOptionPane.showInternalConfirmDialog(content, vbox,
				...
				if (r != JOptionPane.NO_OPTION) {
					...
					fuse = false;
			} else if (we.getID() == WindowEvent.WINDOW_ICONIFIED && !fuse
					&& !fuse2) {
				fuse2 = true; // oneshot....
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == timer && timer != null) {
			...
			if (selectedFrame != null
					&& selectedFrame.getTitle() == CONFIRM_TITLE) // check if
																	// toplevel
																	// is
																	// confirm
																	// dialog.
	...
	private boolean weetJeHetZeker() {
		int r = JOptionPane.showInternalConfirmDialog(content,
				TextMapper.getText(TextMapper.FSD_SURE), CONFIRM_TITLE,
	...
	public void tearDown() {
		...
		JOptionPane.showInternalMessageDialog(content,
				TextMapper.getText(TextMapper.FSD_TIMEUP));
		setVisible(false);
	}
