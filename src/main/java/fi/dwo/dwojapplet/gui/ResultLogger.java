...
public class ResultLogger extends JPanel implements ActionListener,
	...
	private static final String[] LOG_BUTTONS = { BUTTON_LOGANSWERS,
			BUTTON_LOGSCORES, BUTTON_LOGERRORCOUNT, BUTTON_LOGATTEMPTSCOUNT,
			BUTTON_LOGATTEMPTS };
	private static final String TAB = "\t";

	private static final String LOGKEY_LABEL = "logIDLabel";
	...
	private static final String[] LOGKEYS = { LOGKEY_ANSWER, LOGKEY_SCORE,
			LOGKEY_ERRORCOUNT, LOGKEY_ATTEMPTSCOUNT, LOGKEY_ATTEMPTS };
	...
	private String logModeKey = LOGKEY_ANSWER;
	...
	public ResultLogger(Sco sco, SchoolClass schoolClass) {
		...
		// btn = new
		// JButton(TextMapper.getText(TextMapper.GUIRS_BTN_COPY_TO_CLIPBOARD));
	...
	private void requestPartialScore() {
		...
		SortedSet pages = new TreeSet(new Comparator() {
			...
			public int compare(Object o1, Object o2) { // if it looks like a
														// duck, sort like a
														// duck
		...
		for (int i = 0; i < leerlingen.length; i++) {
			...
			for (Iterator iterator = pages.iterator(); iterator.hasNext(); j += 2) {
				...
				partialModel.setValueAt(map.get(PartialScoreIF.SCORE_RAW),
						i + 1, j);
		...
		for (Iterator iterator = pages.iterator(); iterator.hasNext(); j += 2) {
			...
			columnModel.getColumn(j + 1).setHeaderValue(
					TextMapper.getText("tijdsduur"));
		}
			...
			for (int i = 0; i < leerlingen.length; i++) {
				...
				while (iterator.hasNext()) {
					...
					if (value instanceof Map
						...
						model.setValueAt(value, 0, index);
				}
			}
		} catch (Exception e1) {
			alert(e1);
	...
	public void actionPerformed(ActionEvent e) {
		if (BUTTON_REFRESH == e.getActionCommand()) {
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				if (logModeKey.equals(PARTIAL))
				...
				} else if (logModeKey.equals(LOGKEY_ATTEMPTS)) {
					try {
						...
						for (int i = 1; i < len; i++) {
							...
							out.print(value instanceof User ? ((User) value)
									.getUsername() : "");
							out.print(TAB);
							...
							for (int j = 0; j < width; j++) {
								...
								if (value != null && value instanceof Hashtable) {
									...
									boolean correct = score.intValue() > 0
											&& score.intValue() == maxScore
													.intValue();
									out.print(correct);
									out.print(TAB);
					...
					try {
						...
						for (int i = 0; i < len; i++) {
							...
							out.print(name instanceof User ? ((User) name)
									.getUsername() : "");
							out.print(TAB);
	...
	private Map getLog(String suspendData) {
		if (suspendData != null && suspendData.startsWith("{")) // isJSON?
		{
			...
			try {
				Map m = (Map) parser.parse(suspendData);
				return (Map) m.get("log");
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (value != null) {
				...
				if (value instanceof Map) {
					...
					if (logMap.containsKey(LOGKEY_ANSWER))
						answer = (String) logMap.get(LOGKEY_ANSWER);
					else
						hasAnswer = false;
					if (logMap.containsKey(LOGKEY_LABEL))
						label = (String) logMap.get(LOGKEY_LABEL);
					if (logMap.containsKey(LOGKEY_SCORE))
						score = ((Number) logMap.get(LOGKEY_SCORE)).intValue();
					else
						hasScore = false;
					if (logMap.containsKey(LOGKEY_MAXSCORE))
						maxScore = ((Number) logMap.get(LOGKEY_MAXSCORE))
								.intValue();
					else
						hasMaxScore = false;
					if (logMap.containsKey(LOGKEY_ERRORCOUNT))
						errorCount = ((Number) logMap.get(LOGKEY_ERRORCOUNT))
								.intValue();
					else
						hasErrorCount = false;
					if (logMap.containsKey(LOGKEY_ATTEMPTSCOUNT))
						attemptsCount = ((Number) logMap
								.get(LOGKEY_ATTEMPTSCOUNT)).intValue();
					else
						hasAttemptsCount = false;
					if (logMap.containsKey(LOGKEY_ATTEMPTS))
						attempts = (List) logMap.get(LOGKEY_ATTEMPTS);
					else
						hasAttempts = false;

					float f = 0;
					if (maxScore != 0)
						f = (float) score / (float) maxScore;
					...
					if (logModeKey == LOGKEY_ANSWER && hasAnswer) {
						if (row == 0) {
							...
							if (label.indexOf(";") == -1) {
								...
								String[] regels = label.split(";");
								System.out.println("regels:" + regels);
								String text = "<html>";
								for (int i = 0; i < regels.length; i++) {
									text = text + regels[i] + "<BR>";
								}
								text = text + "</html>";
								setText(text);
							}
						} else {
							// if(hasScore && (!hasAttemptsCount ||
							// attemptsCount>0))
							if (maxScore > 0)
								setBackground(new Color(red, green, blue));
								...
								setBackground(new Color(200, 225, 250));
							setText(answer);
							if ("|".equals(answer.trim()))
								setText("");
							if (hasAttempts) {
								...
								for (int i = 0; i < attempts.size(); i++) {
									String newText = (String) attempts.get(i);
									text = text
											+ newText.substring(0,
													newText.indexOf(";"))
											+ "<br>";
							...
							if (answer.startsWith("<math")) {
								mathpane.setText("<html><p style='text-align: center'>"
										+ answer + "</p></html>");
					...
					if (logModeKey == LOGKEY_ERRORCOUNT && hasErrorCount) {
						if (row == 0) {
							...
							if (hasScore
									&& (!hasAttemptsCount || attemptsCount > 0))
								setBackground(new Color(red, green, blue));
							setText("" + errorCount);
						}
					}
					...
					if (logModeKey == LOGKEY_ATTEMPTS && hasAttempts) {
						if (row == 0) {
							...
							setText(text);
							/*
							 * if(attempts.size()>4) { JPanel panel = new
							 * JPanel(); JScrollPane scrollPane = new
							 * JScrollPane
							 * (this,JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
							 * JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
							 * //panel.setMaximumSize(new Dimension(80,50));
							 * panel.add(scrollPane);
							 * scrollPane.setPreferredSize(new
							 * Dimension(120,50)); scrollPane.revalidate();
							 * return panel; }
							 */
						}
					}
	...
	public class LogEditor extends AbstractCellEditor implements
			TableCellEditor {
		...
		public Object getCellEditorValue() {
			if (value != null && value instanceof Hashtable) {
				int score = ((Integer) ((Hashtable) value).get("logScore"))
						.intValue();
	...
	public Object getTransferData(DataFlavor flavor) // TODO werkt nog niet goed
		...
		for (int i = 0; i < len; i++) {
			Object name = leerlingModel.getValueAt(i, 0);
			s = s + (name instanceof User ? ((User) name).getUsername() : "");
			s = s + TAB;
			s = s + (name == null ? "" : name.toString());
			for (int j = 0; j < width; j++) {
				s = s + TAB;
				Object value = model.getValueAt(i, j);
				if (value == null)
