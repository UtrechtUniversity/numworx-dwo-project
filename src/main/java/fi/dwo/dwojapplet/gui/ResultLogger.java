...
public class ResultLogger extends JPanel implements ActionListener, ClipboardOwner, Transferable {
    ...
    public ResultLogger(Sco sco, SchoolClass schoolClass) {
        ...
        //btn = new JButton(TextMapper.getText(TextMapper.GUIRS_BTN_COPY_TO_CLIPBOARD));
        ...
        try {
            ...
            for (int i = 0; i < leerlingen.length; i++) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (value instanceof Hashtable && ((Hashtable) value).containsKey(LOGKEY_MAXSCORE)) {
            ...
            if (value != null) {
                ...
                if (value instanceof Hashtable) {
                    ...
                    if (logMap.containsKey(LOGKEY_SCORE)) {
                        score = ((Integer) logMap.get(LOGKEY_SCORE)).intValue();
                    } else {
                        hasScore = false;
                    }
                    if (logMap.containsKey(LOGKEY_MAXSCORE)) {
                        maxScore = ((Integer) logMap.get(LOGKEY_MAXSCORE)).intValue();
                    } else {
                        hasMaxScore = false;
                    }
                    if (logMap.containsKey(LOGKEY_ERRORCOUNT)) {
                        errorCount = ((Integer) logMap.get(LOGKEY_ERRORCOUNT)).intValue();
                    } else {
                        hasErrorCount = false;
                    }
                    if (logMap.containsKey(LOGKEY_ATTEMPTSCOUNT)) {
                        attemptsCount = ((Integer) logMap.get(LOGKEY_ATTEMPTSCOUNT)).intValue();
                    } else {
                        hasAttemptsCount = false;
                    }
                    if (logMap.containsKey(LOGKEY_ATTEMPTS)) {
                        attempts = (Vector) logMap.get(LOGKEY_ATTEMPTS);
                    } else {
                        hasAttempts = false;
                    }
                    ...
                    if (logModeKey == LOGKEY_ANSWER && hasAnswer) {
                        if (row == 0) {
                            ...
                            if (label.indexOf(";") == -1) {
                                ...
                                System.out.println("index:" + label.indexOf(";"));
                            } else {
