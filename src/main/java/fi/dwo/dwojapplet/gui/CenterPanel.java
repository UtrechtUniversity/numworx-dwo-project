...
/**
 * The main-centerpanel. This can be showed with or without menu. If there is a
 * menu (e.g. te course overview) the CenterMainSubPanel is showed. Otherwise
 * (e.g. the panel with the sco), no menu is showed and the hole center is used.
 *
 * @author M.J.B. Kupers TODO major refactoring
 */
...
//        repaint();
    ...
    public void loadTotal(CenterSubPanel panel) {
        if (!iconizer) {
            ...
            if (GuiConstants.getDwoProfile() == 51 || GuiConstants.getDwoProfile() == 27) {
                setBorder(BorderFactory.createEmptyBorder(14, 28, 76, 30));
            }
            if (GuiConstants.getDwoProfile() == 57 || GuiConstants.getDwoProfile() == 65) {
                setBorder(BorderFactory.createEmptyBorder(0, 28, 54, 30));
            }
        }
        if (tree != null) {
            tree.select(panel.getUserObject());
...
// synchronize the ModuleTreePanel
    ...
    public void setStrategy(SelectStrategy selector) {
        if (tree != null) {
            ...
            if (selector instanceof NullStrategy) {
