package nl.uu.fi.dwo.mobile.promise.client;
...
public class PromiseImpl<T> implements Promise<T> {
    ...
    private final List<ScheduledCommand> tasks = new ArrayList<ScheduledCommand>();
    private int count = 1;
    ...
    public PromiseImpl(Scheduler scheduler) {
    ...
    public Promise<Void> resolveWith(final Promise<? extends T> with) {
        if (with == null)
            throw new NullPointerException();
        final PromiseImpl<Void> result = new PromiseImpl<Void>(scheduler);
    ...
    private synchronized void complete(T value, Throwable failure) {
        ...
        for (ScheduledCommand task : tasks) {
            scheduler.scheduleDeferred(task);
        }
    }
    ...
    public synchronized Promise<T> onResolve(Runnable callback) {
        if (callback == null)
            throw new NullPointerException();
        ScheduledCommand cmd = toCommand(callback);
        if (isDone()) {
	...
	private static ScheduledCommand toCommand(final Runnable callback) {
    ...
    public <R> Promise<R> then(Success<? super T, ? extends R> success, Failure failure) {
        PromiseImpl<R> result = new PromiseImpl<R>(scheduler);
    ...
    public Promise<T> filter(final Predicate<? super T> predicate) {
        if (predicate == null)
            throw new NullPointerException();
        final PromiseImpl<T> result = new PromiseImpl<T>(scheduler);
    ...
    public <R> Promise<R> map(final Function<? super T, ? extends R> mapper) {
        if (mapper == null)
            throw new NullPointerException();
        final PromiseImpl<R> result = new PromiseImpl<R>(scheduler);
