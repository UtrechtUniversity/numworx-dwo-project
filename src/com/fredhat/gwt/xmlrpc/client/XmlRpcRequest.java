 ...
 * have built your {@link XmlRpcClient} object (which contains static
 * information specific to an XML-RPC server such as URL, potential username and
 * password, etc) you will feed this information into the constructor along with
 * the information specific to your method call.
 * 
 * @author Fred Drake
 * 
 * @param &lt;T&gt; The return value type of the method that you are executing
 *        (Integer, String, Map, etc). Legal values are as follows:
 *        <table border=1>
 *        <tr>
 *        <th>Java Type</th>
 *        <th>XML Tag Name</th>
 ...
 *        <td>{@link Integer}</td>
 *        <td>i4 or int</td>
 *        <td>32-bit signed non-null integer value</td>
 *        </tr>
 *        <tr>
 *        <td>{@link Boolean}</td>
 *        <td>boolean</td>
 *        <td>Non-null boolean value (0, or 1)</td>
 *        </tr>
 *        <tr>
 *        <td>{@link String}</td>
 *        <td>string</td>
 ...
 *        <td>{@link Double}</td>
 *        <td>double</td>
 *        <td>A signed, non-null double precision floating point number (64 bit)
 *        </td>
 *        </tr>
 *        <tr>
 *        <td>{@link com.fredhat.gwt.xmlrpc.client.Date}</td>
 *        <td>dateTime.iso8601</td>
 *        <td>A pseudo ISO8601 timestamp</td>
 *        </tr>
 *        <tr>
 *        <td>{@link com.fredhat.gwt.xmlrpc.client.Base64}</td>
 *        <td>base64</td>
 ...
 *        <td>struct</td>
 *        <td>A key value pair. The keys are strings, and the values may be any
 *        valid data type</td>
 *        </tr>
 *        <tr>
 *        <td>{@link java.util.List}</td>
 *        <td>array</td>
 *        <td>An array of objects, which may be of any valid data type</td>
 *        </tr>
 *        <tr>
 *        <td><code>null</code></td>
 *        <td>nil</td>
 *        <td>A null value (only supported from XMLRPC servers with the nil
 *        extension</td>
 *        </tr>
 *        </table>
 * 
...
public class XmlRpcRequest<T> implements Cloneable
{
	 ...
	 * 
	 * @param client
	 *            The object containing XML-RPC server specific information
	 * @param methodName
	 *            The name of the XML-RPC method that you are invoking
	 * @param params
	 *            The array of input parameters for your XML-RPC method. If this
	 *            is null, it will assume no parameters.
	 * @param callback
	 *            The asynchronous callback class instantiation which handles
	 *            business logic on both success and failure of the XML-RPC
	 *            method invocation.
	 */
	 ...
	 * will be in your {@link AsyncCallback} that you defined in the
	 * constructor.
	 */
	public void execute()
	{
		...
		try
		{
			requestBuilder.sendRequest(request.toString(), new RequestCallback()
			{
				public void onResponseReceived(Request req, Response res)
				{
					...
					{
						callback.onFailure(new XmlRpcException("Server returned " + "response code " + res.getStatusCode() + " - " + res.getStatusText()));
						return;
					}
					try
					{
		...
		{
			callback.onFailure(new XmlRpcException("Couldn't make server request.  Are you violating the " + "Same Origin Policy?  Error: " + e.getMessage(), e));
		}
	}
	...
	private T buildResponse(String responseText) throws XmlRpcException
	{
		...
		if (!methodResponse.getNodeName().equals("methodResponse"))
			throw new XmlRpcException("The document element must be named \"methodResponse\" " + "(this is " + methodResponse.getNodeName() + ")");
		if (getElementNodeCount(methodResponse.getChildNodes()) != 1)
			throw new XmlRpcException("There may be only one element under <methodResponse> " + "(this has " + getElementNodeCount(methodResponse.getChildNodes()) + ")");
		...
		if (forkNode.getNodeName().equals("fault"))
		{
			...
			if (faultDetails.get("faultCode") == null || faultDetails.get("faultString") == null || !(faultDetails.get("faultCode") instanceof Integer))
				throw new XmlRpcException("The <fault> element must contain " + "exactly one <struct> with an integer \"faultCode\" and " + "a string \"faultString\" value");
			...
			throw new XmlRpcException("The <methodResponse> must contain either " + "a <params> or <fault> element.");
	...
	private T getValueNode(Node node) throws XmlRpcException
	{
		...
		throw new XmlRpcException("Improper XML-RPC response format:" + " Unknown node name \"" + valueType.getNodeName() + "\"");
	}
